...
//        }
    ...
    static void execute() throws IOException {
        ...
        for (String project : list_projects) {
            cmd = ("NEW_PROJECT " + project + " 0 100").split(" ");
            handle_cmd_query(cmd);
        }

        for (String user : list_users) {
            cmd = ("NEW_USER " + user + " 0 100").split(" ");
            handle_cmd_query(cmd);
        }

        for (String project : list_projects) {
            for (String user : list_users) {
                cmd = ("NEW_PROJECTUSER " + project + " " + user + " 0 100").split(" ");
                handle_cmd_query(cmd);
            }
        }
        for (int i = 0; i < MAX_QUERIES; i++) {
    ...
    private static void handle_cmd_query(String[] cmd) {
        ...
        switch (cmd[0]) {
            ...
            case "NEW_PRIORITY":
                ...
                System.out.println("Time elapsed (ns): " + (qend_time - qstart_time)+"length"+out_user.size());
            ...
            case "NEW_TOP":
                ...
                for(UserReport_ jb:out_topuser) {
                	System.out.println(jb.consumed());
                }
